...
group_levels <- c("Control", "EESREHAB-only")
...
anova_res <- aov_ez(
  ...
  type = 3,
  anova_table = list(
    correction = "none",  # F(14,210)形式で出す
    es = "ges"
  )
)
...
result_table <- anova_table %>%
  mutate(
    Result = paste0(
      ...
      round(F, 2),
      ", P ",
      ifelse(`Pr(>F)` < 0.0001, "< 0.0001", paste0("= ", signif(`Pr(>F)`, 3)))
    )
  ) %>%
...
write.csv(
  result_table,
  "Control_vs_EESREHAB_only_two_way_RM_ANOVA_results.csv",
  row.names = FALSE
)